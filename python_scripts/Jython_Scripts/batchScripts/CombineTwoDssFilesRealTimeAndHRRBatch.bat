...
if "%~3"=="" (
    ...
    echo USAGE: %~nx0 ^<RainfallRealTimePass1And2.dss^> ^<HRR.dss^> ^<RainfallRealTimeFinal.dss^>
    exit /b 1
)
...
echo Combinando RealTime (Pass1And2) y HRR DSS files...
echo   Source 1: %~1
echo   Source 2: %~2
echo   Destino:  %~3
echo.
...
if errorlevel 1 (
    echo **** ERROR: Combinación de RealTime y HRR falló. Revisa el log. ****
    exit /b 1
)

echo.
echo === Combinación de RealTime y HRR completada exitosamente ===
