#!/bin/bash
# MergeGRIBFilesRealTimeHRRBatch.sh - Linux/WSL version
# This script merges HRRR forecast GRIB files using Jython and Vortex

# ===== BASIC CONFIGURATION =====================================
HMS_HOME="/opt/hms"
JYTHON_JAR="/opt/jython.jar"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
JYTHON_SCRIPT="$(dirname "$SCRIPT_DIR")/MergeGRIBFilesRealTimeHRRJython.py"
...
if [ $? -ne 0 ]; then
    echo "**** ERROR: HRRR merge failed. Check the log. ****"
    exit 1
fi
