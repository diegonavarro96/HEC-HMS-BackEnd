...
try:
    ...
    realtime_dss_path_pass1 = resolve_path(config, 'merge_grb', 'output_dss_path_pass2')
    realtime_dss_path_pass2 = resolve_path(config, 'merge_grb', 'output_dss_path')
    ...
    output_dss_dir = os.path.dirname(realtime_dss_path_pass2)
    # Ensure the output directory exists
    os.makedirs(output_dss_dir, exist_ok=True)

    # Define the final combined DSS filename
    combined_dss_filename = "RainfallRealTimePass1And2.dss"
    ...
    combine_jython_script_path = os.path.join(temp_dir, "combine_specific_dss_pass1_pass2.py")
    combine_batch_script_path = os.path.join(temp_dir, "combine_specific_dss_pass1_pass2.bat")

    # --- Log paths for verification ---
    logger.info(f"[Config] HEC-HMS Path: {hms_executable_path}")
    logger.info(f"[Config] Jython Executable: {jython_exe_path}")
    logger.info(f"[Config] Input HRRR DSS: {realtime_dss_path_pass1}")
    logger.info(f"[Config] Input Realtime DSS: {realtime_dss_path_pass2}")
...
input_dss_files = [
    {repr(realtime_dss_path_pass1)},
    {repr(realtime_dss_path_pass2)}
]
...
def run_combine_dss_pass1_pass2():
    """
    ...
    # Pre-run checks
    if not os.path.exists(realtime_dss_path_pass1):
         logger.error(f"Input HRRR DSS file not found: {realtime_dss_path_pass1}. Cannot run combine.")
         return False
    if not os.path.exists(realtime_dss_path_pass2):
         logger.error(f"Input Realtime DSS file not found: {realtime_dss_path_pass2}. Cannot run combine.")
         return False
