...
def build_jython_command(
    ...
    vars_list = merge_cfg.get("variablesv3") # Use only variablesv3 for this script
    out_dss = merge_cfg.get("output_dss_path")

    if not shp_key or not vars_list or not out_dss:
        raise KeyError("merge_grb config missing: clip_shapefile_name, variablesv3, or output_dss_path")
    if not isinstance(vars_list, list):
        raise ValueError("merge_grb.variablesv3 must be a list.")
...
try:
    ...
    for folder in input_folders:
        ...
        for filename in os.listdir(folder):
            if "01H_Pass1" in filename and filename.lower().endswith((".grb2", ".grib2")):
...
"""
    ...
    cmd = [
        ...
        f"-J-Xmx{jython_cfg.get('max_heap','8192m')}", # Increased default max_heap
