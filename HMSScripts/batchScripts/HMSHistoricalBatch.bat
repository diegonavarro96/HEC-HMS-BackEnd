...
rem Usage: HMSHistoricalBatch.bat <script_path> <hms_models_directory>
...
if "%~1"=="" (
    ...
    echo Usage: HMSHistoricalBatch.bat ^<script_path^> ^<hms_models_directory^>
    exit /b 1
)
...
set "HMS_HISTORICAL_MODELS_DIR=%HMS_MODELS_DIR%"

cd /d "%HMS_HOME%"

"%HMS_EXECUTABLE%" -s "%SCRIPT_PATH%" 
